...
class Cam:
    ...
    ''' Constructor '''
    def __init__(self, alpha, beta,theta,c):
        self.PT = [alpha,beta]  # pan and tilt angles
        self.theta = theta # half-angle of view (zoom) [rad]
        self.center = c # optical center
        self.resolution = [300,300] # image sensor resolution (supposed to be squared) [pixels, pixels]

    @property
    def f(self):
        return self.resolution[0] /(2*math.tan(self.theta))
    ...
    # ''' automatically compute camera FoV based on actual center value '''
    # def __setattr__(self, key, value):
    #     try:
    #         self.__dict__[key] = value
    #         if key == "PT" or key == "theta":
    #             self.R = self.compute_R() # rotation wrt world reference frame
    #             self.f = self.resolution[0] /(2*math.tan(self.theta))  # focal length
    #             G = np.hstack((self.R,self.center))
    #             self.K = np.array([[self.f,0,0],[0,self.f,0],[0,0,1]]) # intrinsics matrix (neglect the pixel/mm transformation)
    #             self.P = np.dot(self.K,G)
    ...
    def compute_FoV(self):
        ...
        vertices = np.array([[-self.resolution[0]/2,-self.resolution[0]/2], \
                            [-self.resolution[0]/2,self.resolution[0]/2],\
                            [self.resolution[0]/2,self.resolution[0]/2],\
                            [self.resolution[0]/2,-self.resolution[0]/2]])
