...
name_config =""#"scenario0/strategy0/user_params" + str(10)
if name_config != "":
    user_params = dataImporter.load_obj(dir_path_configurations+name_config)
    print("Loaded configuration: " + name_config)
else:
    print("No configuration loaded")

# load pre-defined agent motion
dir_path_agent_motions = "./data/agent_motions/"
name_agent_motion =""# "scenario0/strategy0/agent_motion" + str(10)
...
try:
    ...
    user_params = {
        ...
        # 'p_init': np.array([[10.0],[10.0],[0.0]]),
        'p_init' :  np.array([ [np.random.uniform(50.0,50.0)], [np.random.uniform(50.0,50.0)] , [0.0] ]), # [m]; initial target position
        # ... parameters of the stochastic input of the underlying motion law (supposed to be biased brownian motion)
        'mu_q' : 0.0, # mean of the driving stochastic input q
        'sigma_q' : 1.0E-7, # variance of the driving stochastic input q 
        ...
        'E_tot' : 300, # total energy capacity 
        # ... Rx
        ...
        # CONTROLLER
        'K' : [1*1.0E-1, 0*5*1.0E-4,0*5*1.0E-4, 1*1.0E-0], # control gains: K_c, K_alpha, K_beta, K_f

        # RBE MAP GENERATOR
        'N_s' : 500, # number of particles
        ...
        # SCENARIO
        'scenario' : 0, # 0 -> NO ENERGY CONSTRAINT, 1 -> CAMERA CAN NOT MOVE, 2- >  DRONE CAN NOT MOVE, 3 -> FULL
        'sim_duration': 100, # simulation duration

        # SEARCH STRATEGY
        'strategy' : 0 # 0 -> RF, 1-> V, 2 -> RF+V
    }
...
# store estimation performance (distance PF estimate to true target position - bias)
est_performance = []
...
plotter = Plotter(agent, uav, pf) # object to plot the environment
for t in tqdm(range(sim_duration)): 

    plt.cla() # clear previous drawings    
    ...
    if name_agent_motion == "":
        ...
        agent.p = agent_motion[t]
    agent_motion.append(agent.p) # save movement
...
plt.ioff() # stop "online" plot

# plot results
plotter.plot_results(Tx_data, Rx_data, uav_energy_level, est_performance, control_cost_functions, vision_events, est_variance)
